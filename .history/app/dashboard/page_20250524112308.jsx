    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
        {statsCards.map((card, index) => (
          <div 
            key={card.title}
            className="bg-white p-4 sm:p-6 rounded-lg shadow-md card-hover animate-slide-in"
            style={{ animationDelay: `${index * 0.1}s` }}
          >
            <div className="flex items-center">
              <div className="p-3 rounded-full bg-indigo-50 hover:bg-indigo-100 smooth-transition">
                {card.icon}
              </div>
              <div className="ml-4">
                <p className="text-xs sm:text-sm text-gray-500 mb-1">{card.title}</p>
                <p className="text-xl sm:text-2xl font-bold text-gray-800 animate-pulse-slow">{card.value}</p>
              </div>
            </div>
          </div>
        ))}
