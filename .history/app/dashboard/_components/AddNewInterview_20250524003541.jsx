...
import { MockInterview, Interview } from "@/utils/schema";
...
function AddNewInterview() {
  ...
  const handleSubmit = async (e) => {
    ...
    const inputPrompt = `Generate 5 interview questions for a ${jobPosition} position with ${jobExperience} years of experience. The tech stack/job description is: ${jobDescription}. Return response in JSON format with array of objects containing question and answer fields.`;
    ...
    try {
      ...
      const interviewResponse = JSON.parse(cleanedResponse);
      
      const res = await db.insert(Interview)
        .values({
          interviewId: uuidv4(),
          jsonResponse: JSON.stringify(interviewResponse),
      ...
      router.push(`dashboard/interview/${res[0]?.interviewId}`);
    } catch (error) {
